...
{{end}}
    ...
    <div class="media">
      <div class="media-body">
        ...
        <p>{{$v.Summary}}</p>
		{{if $.isLogin }}<p class="pull-right">{{if eq $v.Status 0}}<button type="button" class="btn btn-primary btn-xs">屏蔽</button>{{end}} <a href="/article/edit/{{$v.Id}}" class="btn btn-danger">修改</a><a href="/article/remove/{{$v.Id}}" class="btn btn-danger">删除</a></p>{{end}}
      </div>
    </div>
    {{end}}
