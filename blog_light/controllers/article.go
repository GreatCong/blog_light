...
func (this *AddArticleController) Post() { //似乎前后端通过json进行交互的
	if !this.isLogin {
...
func (this *EditArticleController) Get() {
	...
	idstr := this.Ctx.Input.Param(":id") //beego获取用户输入的参数(:id)
...
//删除blog
type RemoveArticleController struct {
	BaseController
}

func (this *RemoveArticleController) Get() {
	if !this.isLogin {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "请先登录"}
		this.ServeJSON()
		return
	...
	// 	this.Redirect("/404.html", 302)
	// }
	err = RemoveArticle(id) //删除博客
	if err == nil {
		// this.Data["json"] = map[string]interface{}{"code": 0, "message": "博客删除成功"}

		// this.TplName = "article.tpl"
		//remove之后就直接调用list的方法
		...
		// this.Data["json"] = map[string]interface{}{"code": 0, "message": "博客删除出错"}
		this.TplName = "index.tpl"
	}
	// this.ServeJSON()
}

func (this *RemoveArticleController) Post() {
	// id, err := this.GetInt("id")

	// err = RemoveArticle(id)

	// if err == nil {
	// 	this.Data["json"] = map[string]interface{}{"code": 1, "message": "博客删除成功", "id": id}
	// } else {
	// 	this.Data["json"] = map[string]interface{}{"code": 0, "message": "博客删除出错"}
	// }
	// this.ServeJSON()
	//删除应该用不到post吧
	this.Data["json"] = map[string]interface{}{"code": 0, "message": "博客删除Post"}
	this.ServeJSON()
}
...
func (this *ShowArticleController) Get() {
	...
	this.TplName = "article-detail.tpl" //设置页面的模板
}
